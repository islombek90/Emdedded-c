 ...
 *
 * @brief          - This function enables or disables the USART  clock
 *
 * @parama[in]     - the base address of USART port peripheral
 ...
 *
 * @brief          - This function controls the Baudrate selection
 *
 * @parama[in]     - the base address of USART port peripheral
 * @parama[in]     - Baudrate value
 * @parama[in]     -
 *
 * @return         -None
 *
...
void USART_SetBaudRate(USART_RegDef_t *pUSARTx, uint32_t BaudRate)
{
  ...
  if(pUSARTx == USART1 || pUSARTx == USART2)
  {
 ...
 * @fn             -  USART_Init
 *
 * @brief          - This function inits the USART
 *
 * @parama[in]     -
 * @parama[in]     -
 * @parama[in]     -
...
/******************************** Configuration of CR1******************************************/
	...
	//Implement the code to enable the Clock for given USART peripheral
	 USART_PeriClkControl(pUSARTHandler->pUSARTx,ENABLE);
	...
	tempreg |= pUSARTHandler->USART_Config.USART_WordLength << USART_CR1_M0 ;
   ...
   //Program the CR1 register
	pUSARTHandler->pUSARTx->USART_CR1 = tempreg;
...
/******************************** Configuration of CR2******************************************/
	...
	tempreg |= pUSARTHandler->USART_Config.USART_StopBitMode << USART_CR2_STOP;

	//Program the CR2 register
	pUSARTHandler->pUSARTx->USART_CR2 = tempreg;
...
/******************************** Configuration of CR3******************************************/
	...
	pUSARTHandler->pUSARTx->USART_CR3 = tempreg;

/******************************** Configuration of BRR(Baudrate register)******************************************/

	//Implement the code to configure the baud rate
	//We will cover this in the lecture. No action required here
	USART_SetBaudRate(pUSARTHandler->pUSARTx,pUSARTHandler->USART_Config.USART_BaudRate);
...
void USART_SendData(USART_handler_t  *pUSARTHandle, uint8_t *pTxBuffer, uint32_t Len)
{
   ...
   //Loop over until "Len" number of bytes are transferred
	for(uint32_t i = 0 ; i < Len; i++)
	{
		...
		if(pUSARTHandle->USART_Config.USART_WordLength == USART_WORD_LENGTH_9)
		{
		...
		for(uint32_t i = 0 ; i < Len; i++)
		{
			...
			else
			{
				...
				if(pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE)
				{
					...
					//read 8 bits from DR
					 *pRxBuffer = (uint8_t) (pUSARTHandle->pUSARTx->USART_RDR  & (uint8_t)0xFF);
				}
 ...
 * @Note              -
 */
uint8_t USART_SendDataIT(USART_handler_t *pUSARTHandle,uint8_t *pTxBuffer, uint32_t Len)
{
 ...
 * @Note              -
 */
uint8_t USART_ReceiveDataIT(USART_handler_t *pUSARTHandle,uint8_t *pRxBuffer, uint32_t Len)
{
 ...
 * @Note              -
 */
void USART_IRQHandling(USART_handler_t *pUSARTHandle)
{
		...
		if(pUSARTHandle->TxBusyState == USART_BUSY_IN_TX)
		{
			...
			if(pUSARTHandle->TxLen > 0)
			{
				...
				if(pUSARTHandle->USART_Config.USART_WordLength == USART_WORD_LENGTH_9)
				{
					...
					pUSARTHandle->pUSARTx->USART_RDR = (*pdata & (uint16_t)0x01FF);
				...
				{
					//This is 8bit data transfer
					pUSARTHandle->pUSARTx->USART_RDR = (*pUSARTHandle->pTxBuffer  & (uint8_t)0xFF);
 ...
 * @Note              -
 */
__weak void USART_ApplicationEventCallback(USART_handler_t *pUSARTHandle,uint8_t event)
{
